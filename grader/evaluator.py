...
Updates the frontend after each test case, but not more often than 0.33 sec (with the last test
being an exception).
"""
...
class Evaluator:
    def __init__(self, data):
        ...
        # Front-end endpoint
        self.update_url = data["updateEndpoint"].rstrip("/")
        ...
        self.tests_url = data["testsEndpoint"].rstrip("/")
        ...
        if "checker" in data and data["checker"] != "":
            self.path_checker_source = os.path.join(
                config.PATH_CHECKERS, data["checker"] + config.SOURCE_EXTENSION_CPP
            )
            self.path_checker_executable = os.path.join(
                config.PATH_CHECKERS, data["checker"] + config.EXECUTABLE_EXTENSION_CPP
            )
        ...
        if "tester" in data and data["tester"] != "":
            self.path_tester_source = os.path.join(
                config.PATH_TESTERS, data["tester"] + config.SOURCE_EXTENSION_CPP
            )
            self.path_tester_executable = os.path.join(
                config.PATH_TESTERS, data["tester"] + config.EXECUTABLE_EXTENSION_CPP
            )
        ...
        self.path_sandbox = os.path.join(
            config.PATH_SANDBOX, "submits", "submit_{:06d}/".format(self.id)
        )
        self.path_source = os.path.join(
            self.path_sandbox, config.SOURCE_NAME + common.get_source_extension(self.language)
        )
        self.path_executable = os.path.join(
            self.path_sandbox,
            config.EXECUTABLE_NAME + common.get_executable_extension(self.language),
        )
    ...
    def evaluate(self):
        ...
        logger.info(
            "Submit {id} | Evaluating submit {submit_id}".format(id=self.id, submit_id=self.id)
        )
        ...
        if not self.create_sandbox_dir():
            self.updater.add_info(
                message="Error while creating sandbox directory!", status=TestStatus.INTERNAL_ERROR
            )
            return
        ...
        if not self.download_tests():
            self.updater.add_info(
                message="Error while downloading test files!", status=TestStatus.INTERNAL_ERROR
            )
            return
        ...
        if self.path_checker_executable is not None:
            if not os.path.exists(self.path_checker_executable):
                logger.info(
                    "Submit {id} |   >> downloading and compiling checker...".format(id=self.id)
                )
                if not self.setup_utility_file(
                    self.path_checker_source, self.path_checker_executable, self.checker_url
                ):
                    self.updater.add_info(
                        message="Error while setting up checker!", status=TestStatus.INTERNAL_ERROR
                    )
                    return
        ...
        if self.path_tester_executable is not None:
            if not os.path.exists(self.path_tester_executable):
                logger.info(
                    "Submit {id} |   >> downloading and compiling tester...".format(id=self.id)
                )
                if not self.setup_utility_file(
                    self.path_tester_source, self.path_tester_executable, self.tester_url
                ):
                    self.updater.add_info(
                        message="Error while setting up tester!", status=TestStatus.INTERNAL_ERROR
                    )
                    return
        ...
        if not self.run_solution():
            self.updater.add_info(
                message="Error while processing the solution!", status=TestStatus.INTERNAL_ERROR
            )
            return
    ...
    def create_sandbox_dir(self):
        try:
            ...
            logger.error(
                "Submit {id} | Could not create sandbox directory. Exception was: {ex}".format(
    ...
    def download_test(self, file_name, file_hash, file_path):
        ...
        if not os.path.exists(file_path):
            logger.info(
                "Submit {id} | Downloading file {file_name} with hash {file_hash} from URL: {url}".format(
                    id=self.id,
                    file_name=file_name,
                    file_hash=file_hash,
    ...
    def download_tests(self):
        ...
        try:
            ...
            logger.error(
                "Submit {id} | Could not download tests. Exception was: {ex}".format(
                    id=self.id, ex=str(ex)
                )
            )
            return False
    ...
    def compile(self, language, path_source, path_executable, print_error=False):
        try:
            ...
            if compile_status != "":
                ...
                error_log_func(
                    "Submit {id} | Could not compile file {file_path}! Compiler status: {compile_status}".format(
                        id=self.id, file_path=path_source, compile_status=compile_status
                    )
                )
            ...
            logger.error(
                "Submit {id} | Could not compile file {file_path}! Exception was: {ex}".format(
    ...
    def write_source(self, source, destination):
        try:
            ...
            logger.error(
                "Submit {id} | Could not write source file. Exception was: {ex}".format(
                    id=self.id, ex=str(ex)
                )
            )
            return False
    ...
    def process_problem(self):
        ...
        for test in self.tests:
            future = common.job_pool.submit(
                execute_problem, self.updater, self.id, result_id, test, run_config
            )
        ...
        for test, future in test_futures:
            try:
                ...
                logger.error(
                    "Submit {id} | Exception on test {test_name} ({test_hash}): {ex}".format(
    ...
    def process_game(self):
        ...
        for match in self.matches:
            logger.info(
                "Submit {id} |     -- running game {player1} vs {player2}...".format(
            ...
            opponent_path_executable = os.path.join(
                self.path_sandbox,
                config.OPPONENT_EXECUTABLE_NAME
            ...
            if (
                self.compile(opponent_language, opponent_path_source, opponent_path_executable)
                != ""
            ):
                logger.error(
                    "Submit {id} | Could not compile opponent's source!".format(id=self.id)
                )
                continue
        ...
        logger.info(
            "Submit {id} |     -- executed {cnt} matches in {time:.3f}s.".format(
    ...
    def run_solution(self):
        if self.problem_type == "interactive" or self.problem_type == "game":
            if self.path_tester_executable is None:
                logger.error(
                    "Submit {id} | Game or interactive problem without a tester!".format(id=self.id)
                )
                return False

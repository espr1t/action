...
class TestConcurrency(TestCase):
    ...
    def test_run_program_concurrent_hdd_access(self):
        ...
        with mock.patch("config.CONCURRENT_IO_LIMIT", 2**30):
            ...
            # This time the worst time should be significantly larger
            print("TIME: {:.3f}s vs {:.3f}s".format(min(times), max(times)))
            self.assertGreater(max(times), max(min(times) + 0.1, min(times) * 1.1))
...
"""
    ...
    """
    # TODO: Figure out why some wait()s are 0.000s and some are more assuming CPU affinity works.
    # The wait()s with time 0.000s are apparently when the Sandbox object is being deleted
    # The wait()s with larger time (2x or even 3x) though are strange.
